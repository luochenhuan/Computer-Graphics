...
// Create a non-hierarchical cuboid node
extern "C"
int gr_cuboid_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    ...
    double depth = luaL_checknumber(L, 5);

    data->node = new GeometryNode(name, new Cuboid(pos, width, height, depth));
...
int gr_nh_cylinder_cmd(lua_State* L)
{
  ...
  Vector3D ori;
  get_tuple(L, 2, &pos[0], 3);
  get_tuple(L, 3, &ori[0], 3);

  double radius = luaL_checknumber(L, 4);
  double height = luaL_checknumber(L, 5);

  data->node = new GeometryNode(name, new NonhierCylinder(pos, ori, radius, height));
...
int gr_nh_cone_cmd(lua_State* L)
{
  ...
  double radius = luaL_checknumber(L, 3);
  double height = luaL_checknumber(L, 4);
  double yend   = luaL_checknumber(L, 5);

  data->node = new GeometryNode(name, new NonhierCone(pos, radius, height, yend));
...
/***************** Make a cone light *************************/
// extern "C"
// int gr_conelight_cmd(lua_State* L)
// {
//   GRLUA_DEBUG_CALL;
//
//
...
//   get_tuple(L, 4, &l.direction[0], 3);
//   l.DirPow = luaL_checknumber(L, 5);
//   l.HalfCos = luaL_checknumber(L, 6);
//
//   l.colour = Colour(col[0], col[1], col[2]);
//
//   data->conelight = new ConeLight(l);
//
//   luaL_newmetatable(L, "gr.conelight");
//   lua_setmetatable(L, -2);
//
//   return 1;
// }
...
// Create a checker
extern "C"
int gr_checker_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    int n = lua_gettop(L);
    gr_material_ud* data = (gr_material_ud*)lua_newuserdata(L, sizeof(gr_material_ud));
    ...
    if (n > 5)
    {
        ...
        if (n > 7)
        {
            std::string nm = luaL_checkstring(L,8);
            if (nm == "pNormalMap")
            {
                double scale = luaL_checknumber(L, 9);
                double amount = luaL_checknumber(L, 10);
                data->material = new Checker(Colour(kd[0], kd[1], kd[2]),
                                            Colour(ks[0], ks[1], ks[2]),
                                            shininess, reflect_ratio, refract_ratio,
                                            Colour(c1[0], c1[1], c1[2]),
                                            Colour(c2[0], c2[1], c2[2]), new pNormalMap(scale, amount));
            }
            else if (nm == "imgNormalMap")
            {
                std::string nmName = luaL_checkstring(L, 9);
                data->material = new Checker(Colour(kd[0], kd[1], kd[2]),
                                            Colour(ks[0], ks[1], ks[2]),
                                            shininess, reflect_ratio, refract_ratio,
                                            Colour(c1[0], c1[1], c1[2]),
                                            Colour(c2[0], c2[1], c2[2]), new imgNormalMap(nmName));
            }
...
extern "C"
int gr_imgtex_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    int n = lua_gettop(L);
    gr_material_ud* data = (gr_material_ud*)lua_newuserdata(L, sizeof(gr_material_ud));
    data->material = 0;
    std::string img_name = luaL_checkstring(L, 1);
    ...
    double reflect_ratio = luaL_checknumber(L, 5);
    double refract_ratio = luaL_checknumber(L, 6);
    if (n > 6)
    {
        std::string nm = luaL_checkstring(L, 7);
        if (nm == "pNormalMap")
        {
            double scale = luaL_checknumber(L, 8);
            double amount = luaL_checknumber(L, 9);
            data->material = new imgTexture(img_name, Colour(kd[0], kd[1], kd[2]),
                                        Colour(ks[0], ks[1], ks[2]),
                                        shininess, reflect_ratio, refract_ratio, new pNormalMap(scale, amount));
        }
        else if (nm == "imgNormalMap")
        {
            std::string nmName = luaL_checkstring(L, 8);
            data->material = new imgTexture(img_name, Colour(kd[0], kd[1], kd[2]),
                                        Colour(ks[0], ks[1], ks[2]),
                                        shininess, reflect_ratio, refract_ratio, new imgNormalMap(nmName));
        }
...
static const luaL_reg grlib_functions[] = {
  ...
  {"checker", gr_checker_cmd},
  {"turbulence", gr_turb_cmd},
  {"imgtex", gr_imgtex_cmd},
  {"cube", gr_cube_cmd},
  {"nh_sphere", gr_nh_sphere_cmd},
  {"nh_box", gr_nh_box_cmd},
  {"cuboid", gr_cuboid_cmd},
  {"nh_cylinder", gr_nh_cylinder_cmd},
  {"nh_cone", gr_nh_cone_cmd},
  {"plane", gr_plane_cmd},
  {"mesh", gr_mesh_cmd},
  {"light", gr_light_cmd},
  // {"conelight", gr_conelight_cmd},
